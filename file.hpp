...
#if !(defined(sfl_alloca) && defined(sfl_freea))
#undef sfl_alloca
#undef sfl_freea
#ifndef SFL_NO_ALLOCA
#include <alloca.h>
#define sfl_alloca(sz) alloca(sz)
#define sfl_freea(ptr)
#else
#define sfl_alloca(sz) malloc(sz)
#define sfl_freea(ptr) free(pr)
#endif
#endif


#ifndef SFL_MAX_STACK_STRING
#define SFL_MAX_STACK_STRING 512
#endif

#ifndef SFL_FAILED_TO_CLOSE
#define SFL_FAILED_TO_CLOSE(EX)
#endif

#define svalloca(SV) sfl_alloca(SV.size() + 1)
...
class File;
using ExpectedFile = tl::expected<File, FileError>;
using ExpectedString = tl::expected<std::string, FileError>;
using ExpectedMode = tl::expected<const char *, FileError>;
using ExpectedVoid = tl::expected<void, FileError>;
...
private:
    ...
    static constexpr auto maxStackString = SFL_MAX_STACK_STRING;
...
public:
    ...
    [[nodiscard]] ExpectedVoid write(const std::string &data) noexcept;
    [[nodiscard]] ExpectedVoid write(const char *data, size_t size) noexcept;
    ExpectedVoid close() noexcept;


private:
    static char *svToCharPtr(void *dest, std::string_view source) noexcept;
    [[nodiscard]] static ExpectedMode stateToMode(uint8_t state) noexcept;
...
#define SFL_TRY(Assign, Ex)                         \
            auto UNIQUE(tmp) = std::move(Ex);       \
            if (!UNIQUE(tmp)){                      \
                return tl::unexpected(UNIQUE(tmp).error()); \
            }                                       \
            Assign = std::move(UNIQUE(tmp).value());        \

#define SFL_TRY_VOID(Ex)                        \
    ({                                          \
        auto tmp = Ex;                          \
        if (!tmp) {                             \
...
File::~File() noexcept {
    if(const auto ok = close(); !ok){
        SFL_FAILED_TO_CLOSE(ok);
    }
}

ExpectedFile File::open(std::string_view filename, uint8_t state) noexcept {
    char *name = svToCharPtr(svalloca(filename), filename);

    SFL_TRY(const char *mode, stateToMode(state));
    if (!mode) {
        return tl::unexpected(FileError {"failed to set mode from provided state", 0});
    }

    FILE *fp = std::fopen(name, mode);
    if (!fp) {
        return tl::unexpected(FileError {"Could not open file", errno});
    }
    sfl_freea(name);
    return File {fp};
}
...
ExpectedString File::read() noexcept {
    std::fseek(m_fp, 0l, SEEK_END);
    const size_t fsize = static_cast<size_t>(std::ftell(m_fp));
    std::rewind(m_fp);

    std::string out(fsize, '\0');
    const auto ret = std::fread(out.data(), sizeof(char), fsize, m_fp);
    if (ret < fsize) {
...
ExpectedVoid File::write(std::string_view data, bool simple) noexcept {
    if(simple){
...
ExpectedVoid File::write(const std::string &data) noexcept {
    return write(data.c_str(), data.size());
}

ExpectedVoid File::write(const char *data, size_t size) noexcept {
...
ExpectedVoid File::close() noexcept{
    if (m_fp == nullptr) {
        return{};
    }
    if (std::fclose(m_fp)) {
        return tl::unexpected(FileError {"Could not close file", errno});
    }
    return{};

}

ExpectedMode File::stateToMode(uint8_t state) noexcept {
    static char m[4] = {0};
    auto *it = std::begin(m);
    const auto stateRep = *reinterpret_cast<StateRepr *>(&state);
    if (stateRep.padding || (stateRep.write && stateRep.append) || !(stateRep.read || stateRep.write || stateRep.append)) {
        return tl::unexpected(FileError {"Illegal state for opening a file"});
    }
...
char *File::svToCharPtr(void *dest, std::string_view source) noexcept {
    std::memcpy(dest, &*source.begin(), source.size() * sizeof(std::string_view::value_type));

    static_cast<char *>(dest)[source.size()] = 0;
    return static_cast<char *>(dest);
}

ExpectedString File::readFile(std::string_view filename) noexcept {
    SFL_TRY(File f,File::open(filename, File::Read));
    SFL_TRY(const auto ret, f.read());
    return ret;
}

ExpectedVoid File::writeFile(std::string_view filename, std::string_view data) noexcept {
    SFL_TRY(File f, File::open(filename, File::Write));
    SFL_TRY_VOID(f.write(data));
    return {};
}
